// controllers/native/nativeGuideAvailabilityController.js
import mongoose from "mongoose";
import Guide from "../../models/guideModel.js";
import GuideAvailability from "../../models/guideAvailabilityModel.js";
import { handleError, sendErrorResponse, ErrorCodes } from "../../utils/nativeErrors.js";
 ...
 * GET /api/native/guides/:id/availability?from=YYYY-MM-DD&to=YYYY-MM-DD
...
export const getGuideAvailability = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(guideId)) {
      return sendErrorResponse(res, ErrorCodes.INVALID_FIELD_VALUE, "Invalid guide ID");
    }

    if (!from || !to) {
      return sendErrorResponse(res, ErrorCodes.MISSING_REQUIRED_FIELD, "Query params 'from' and 'to' (YYYY-MM-DD) are required");
    }

    const fromDate = new Date(from);
    const toDate = new Date(to);
    if (isNaN(fromDate.getTime()) || isNaN(toDate.getTime())) {
      return sendErrorResponse(res, ErrorCodes.INVALID_FIELD_VALUE, "Invalid date format. Use YYYY-MM-DD");
    }

    const start = toStartOfDayUTC(fromDate);
    const end = toStartOfDayUTC(toDate);
    if (start > end) {
      return sendErrorResponse(res, ErrorCodes.VALIDATION_ERROR, "'from' must be before or equal to 'to'");
    }

    const guide = await Guide.findById(guideId);
    if (!guide) {
      return sendErrorResponse(res, ErrorCodes.RESOURCE_NOT_FOUND, "Guide not found");
    }
 ...
 * PUT /api/native/guides/:id/availability
  ...
  try {
    ...
    if (date) {
      ...
      if (isNaN(start.getTime()) || isNaN(end.getTime())) {
        return sendErrorResponse(res, ErrorCodes.INVALID_FIELD_VALUE, "Invalid startDate or endDate. Use YYYY-MM-DD");
      }
      const from = toStartOfDayUTC(start);
      const to = toStartOfDayUTC(end);
      if (from > to) {
        return sendErrorResponse(res, ErrorCodes.VALIDATION_ERROR, "startDate must be before or equal to endDate");
      }
      ops.push({
        filter: {
          guide: new mongoose.Types.ObjectId(guideId),
      ...
      for (const dStr of dates) {
        ...
        if (isNaN(d.getTime())) {
          return sendErrorResponse(res, ErrorCodes.INVALID_FIELD_VALUE, `Invalid date in array: ${dStr}. Use YYYY-MM-DD`);
        }
        const startOfDay = toStartOfDayUTC(d);
        ops.push({
          filter: {
